...
/**
 * Test: Flunorette\Selections: Aggregation functions.
...
$authors = $connection->table('author')->select('author.*')->where('book:translator_id IS NOT NULL')->group('author.id');  // SELECT `author`.* FROM `author` INNER JOIN `book` ON `author`.`id` = `book`.`author_id` WHERE (`book`.`translator_id` IS NOT NULL) GROUP BY `author`.`id`
