...
test(function() use ($preprocessor) {
	list($sql, $params) = $preprocessor->process(array('SELECT id FROM author WHERE (id IN (?))', array(11, 12)));
	Assert::same('SELECT id FROM author WHERE (id IN (11, 12))', $sql);
	Assert::same(array(), $params);
});


test(function() use ($preprocessor) {
	list($sql, $params) = $preprocessor->process(array('SELECT id = :id FROM author WHERE id = :id OR id = :id', array(':id' => 11)));
	Assert::same('SELECT id = :id FROM author WHERE id = :id OR id = :id', $sql);
	Assert::same(array(':id' => 11), $params);
});
...
test(function() use ($preprocessor) { // missing parameters
	...
	}, 'Flunorette\\InvalidArgumentException', 'There are more placeholders than passed parameters.');
});
