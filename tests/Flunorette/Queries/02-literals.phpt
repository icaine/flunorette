...
Assert::same('SELECT user.* FROM user WHERE (id > ?) ORDER BY name IN (SELECT name FROM user WHERE name IN (?)) OR id IN (?)', $query->getQuery());
...
Assert::same('SELECT user.* FROM user WHERE (id > ?) ORDER BY EXISTS (SELECT * FROM user WHERE name IN (?)) DESC, id IN (?)', $query->getQuery());
Assert::same(array(0, 'Marek', array(1, 2)), $query->getParameters());

$query = $connection->createSelect('user');
$query->where('id > ?', 0)->order('id ?, ?', array(1, 2), new SqlLiteral('EXISTS (SELECT * FROM user WHERE name IN (?)) DESC', array('Marek')));
Assert::same('SELECT user.* FROM user WHERE (id > ?) ORDER BY id IN (?), EXISTS (SELECT * FROM user WHERE name IN (?)) DESC', $query->getQuery());
...
Assert::same('SELECT user.* FROM user WHERE (id > ?) ORDER BY EXISTS (SELECT * FROM user WHERE name IN (?)) DESC', $query->getQuery());
...
Assert::same('SELECT user.* FROM user ORDER BY EXISTS (SELECT * FROM user WHERE name IN (?)) DESC', $query->getQuery());
...
Assert::same('SELECT user.* FROM user ORDER BY name IN (SELECT name FROM user WHERE name = :name)', $query->getQuery());
